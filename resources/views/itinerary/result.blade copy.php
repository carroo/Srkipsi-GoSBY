    ...
    <section class="py-10 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg shadow">
                            <div class="flex items-start">
                                ...
                                <div class="ml-3 flex-1">
                                    <p class="text-yellow-800">{{ is_array($alert) ? ($alert['message'] ?? $alert['title'] ?? '') : $alert }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div id="dpStepsContent" style="display: none;">
                    ...
                    <div class="relative">
                        @foreach ($dpSteps as $step)
                            @php
                                $stepColors = [
                                    'INIT' => ['bg' => 'bg-blue-100', 'border' => 'border-blue-400', 'text' => 'text-blue-800', 'icon' => 'bg-blue-500'],
                                    'LOC_PREP' => ['bg' => 'bg-green-100', 'border' => 'border-green-400', 'text' => 'text-green-800', 'icon' => 'bg-green-500'],
                                    'NO_VALID_ROUTE' => ['bg' => 'bg-red-100', 'border' => 'border-red-400', 'text' => 'text-red-800', 'icon' => 'bg-red-500'],
                                    'FINAL' => ['bg' => 'bg-purple-100', 'border' => 'border-purple-400', 'text' => 'text-purple-800', 'icon' => 'bg-purple-500'],
                                    ...
                                    <div class="border-l-4 {{ $colors['border'] }} {{ $colors['bg'] }} rounded-r-lg p-4 shadow-md">
                                        ...
                                        @if (isset($step['data']) && !empty($step['data']))
                                            <div class="bg-white bg-opacity-60 rounded p-3">
                                                ...
                                                @elseif ($step['step'] === 'LOC_PREP' && isset($step['data']['locations']))
                                                    <div class="space-y-1">
                                                        <strong class="text-xs text-gray-700">Daftar Lokasi (n={{ $step['data']['n'] ?? count($step['data']['locations']) }}):</strong>
                                                        <ul class="text-xs text-gray-600 list-disc list-inside">
                                                            @foreach ($step['data']['locations'] as $loc)
                                                                <li>Index {{ $loc['index'] }}: {{ $loc['name'] }}</li>
                                                            @endforeach
                                                ...
                                                @elseif (($step['step'] === 'FINAL_RESULT' || $step['step'] === 'FINAL') && isset($step['data']['route']))
                                                    <div class="space-y-2">
                                                        <dl class="grid grid-cols-2 gap-2 text-xs">
                                                            @foreach ($step['data'] as $key => $value)
                                                                @if ($key !== 'route' && !is_array($value))
                                                                    <dt class="text-gray-600 font-semibold">{{ ucwords(str_replace('_', ' ', $key)) }}:</dt>
                                                                    <dd class="text-gray-900">{{ $value }}</dd>
                                                                @endif
                                                            @endforeach
                                                        </dl>
                                                        @if(isset($step['data']['route']) && is_array($step['data']['route']))
                                                            <div class="pt-2 border-t">
                                                                <strong class="text-xs text-gray-700">Rute Optimal:</strong>
                                                                <div class="text-sm text-indigo-900 font-bold mt-1">
                                                                    {{ implode(' → ', $step['data']['route']) }}
                                                                </div>
                                                            </div>
                                                        @endif
                                                    </div>
            ...
            @foreach ($route['days'] as $dayData)
                @php
                    // day_of_week is already a string (e.g., "Senin", "Selasa")
                        ...
                        <div class="table-wrapper rounded-lg overflow-hidden">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <!-- Titik Awal (Start Location) untuk setiap hari -->
                                    <tr class="bg-green-50">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <div class="text-sm text-green-600">{{ $dayData['start_location']['name'] ?? 'Lokasi Awal' }}</div>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600">
                                            @if(isset($dayData['start_location']['latitude']) && isset($dayData['start_location']['longitude']))
                                                {{ number_format($dayData['start_location']['latitude'], 6) }}, {{ number_format($dayData['start_location']['longitude'], 6) }}
                                            @else
                                                -
                                            @endif
                                        </td>
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4">
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-blue-100 text-blue-800">
                                                    ...
                                                    {{ number_format(($location['distance_from_prev'] ?? 0) / 1000, 2) }} km
                                                </span>
                                            </td>
                                            <td class="px-6 py-4">
                                    ...
                                    <!-- Titik Akhir (End Location) untuk setiap hari -->
                                    @php
                                        // Untuk hari terakhir, gunakan endLocation dari settings
                                        // Untuk hari lainnya, gunakan end_location dari dayData
                                        $endLoc = $dayData['end_location'] ?? null;
                                        
                                        // Hitung jarak dari lokasi terakhir ke end location
                                        $returnDistance = 0;
                                        $returnDuration = 0;
                                        
                                        if (!empty($dayData['locations']) && $endLoc) {
                                            $lastLocation = end($dayData['locations']);
                                            $lastLocationId = $lastLocation['tourism']['id'] ?? null;
                                            $endLocId = $endLoc['id'] ?? null;
                                            
                                            if ($lastLocationId && $endLocId && isset($distanceMatrix[$lastLocationId][$endLocId])) {
                                                $returnDistance = $distanceMatrix[$lastLocationId][$endLocId]['distance'] ?? 0;
                                                $returnDuration = $distanceMatrix[$lastLocationId][$endLocId]['duration'] ?? 0;
                                            }
                                        }
                                        
                                        // Tentukan warna badge berdasarkan tipe akhir
                                        $isAccommodation = isset($endLoc['id']) && $endLoc['id'] === 'accommodation';
                                        $isFinalEnd = $dayData['day'] === $settings['duration_days'];
                                        $bgColor = $isAccommodation ? 'bg-yellow-50' : ($isFinalEnd ? 'bg-red-50' : 'bg-gray-50');
                                        $badgeColor = $isAccommodation ? 'bg-yellow-500' : ($isFinalEnd ? 'bg-red-500' : 'bg-gray-500');
                                        $textColor = $isAccommodation ? 'text-yellow-800' : ($isFinalEnd ? 'text-red-800' : 'text-gray-800');
                                        $subtextColor = $isAccommodation ? 'text-yellow-600' : ($isFinalEnd ? 'text-red-600' : 'text-gray-600');
                                        $icon = $isAccommodation ? '🏨' : '🏁';
                                        $label = $isAccommodation ? 'LOKASI PENGINAPAN' : 'TITIK AKHIR';
                                    @endphp
                                    
                                    @if($endLoc)
                                        <tr class="{{ $bgColor }}">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="flex items-center justify-center w-8 h-8 {{ $badgeColor }} text-white rounded-full font-bold">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                    </svg>
                                                </span>
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="font-bold {{ $textColor }}">{{ $icon }} {{ $label }}</div>
                                                <div class="text-sm {{ $subtextColor }}">{{ $endLoc['name'] ?? 'Lokasi Akhir' }}</div>
                                            </td>
                                            <td class="px-6 py-4 text-sm text-gray-600">
                                                @if(isset($endLoc['latitude']) && isset($endLoc['longitude']))
                                                    {{ number_format($endLoc['latitude'], 6) }}, {{ number_format($endLoc['longitude'], 6) }}
                                                @else
                                                    -
                                                @endif
                                            </td>
                                            <td class="px-6 py-4">
                                                @if ($returnDistance > 0)
                                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-blue-100 text-blue-800">
                                                        {{ number_format($returnDistance / 1000, 2) }} km
                                                    </span>
                                                @else
                                            ...
                                            <td class="px-6 py-4 text-sm text-gray-500">
                                                @if ($returnDuration > 0)
                                                    {{ date('H:i', $currentTime + ceil($returnDuration / 60) * 60) }}
                                                @else
                                                    {{ $settings['end_time'] }}
                                                @endif
                                            </td>
