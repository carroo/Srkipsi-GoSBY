                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    ...
                    @if($tripCartItems->count() > 0)
                        <div class="space-y-4" id="tripCartList">
                            @foreach($tripCartItems as $item)
                                <div class="bg-gray-50 rounded-lg p-4 hover:shadow-md transition-shadow duration-300 border border-gray-200" data-cart-id="{{ $item->id }}" data-tourism-id="{{ $item->tourism_id }}">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-shrink-0">
                                            ...
                                            @else
                                                <div class="w-26 h-26 bg-gray-300 rounded-lg flex items-center justify-center">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h3 class="text-lg font-semibold text-gray-900 mb-1">{{ $item->tourism->name }}</h3>
                                            <p class="text-sm text-gray-600 mb-2 line-clamp-2">{{ Str::limit($item->tourism->description, 100) }}</p>
                                                ...
                                                @if($item->tourism->prices && $item->tourism->prices->count() > 0)
                                                    ...
                                                    <div class="flex items-center text-sm">
                                                        ...
                                                        @if($minPrice == $maxPrice)
                                                            <span class="font-semibold text-green-600">Rp {{ number_format($minPrice, 0, ',', '.') }}</span>
                                                        @elseif ($minPrice === 0)
                                                            <span class="font-semibold text-green-600">Gratis</span>
                                                        @else
                                                            <span class="font-semibold text-green-600">Rp {{ number_format($minPrice, 0, ',', '.') }} - Rp {{ number_format($maxPrice, 0, ',', '.') }}</span>
                                                        @endif
                                                    </div>
                                                @else
                                                    <div class="flex items-center text-sm">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="text-gray-600 mb-4">Tambahkan destinasi wisata ke trip cart untuk membuat itinerary</p>
                            <a href="{{ route('tourism.index') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors duration-200">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl shadow-md p-6 sticky top-6">
                    ...
                    <form id="itineraryForm" method="POST" action="{{ route('itinerary.generate') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <div id="customLocationSection" style="display: none;">
                                <div class="space-y-3">
                                    ...
                                    <div id="customLocationPreview" class="hidden p-3 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        @if($tripCartItems->count() == 0)
                            <p class="text-xs text-red-600 text-center mt-2">Tambahkan minimal 1 destinasi untuk membuat itinerary</p>
                        @endif
                    </form>
                </div>
            </div>
...
<!-- Map Modal -->
<div id="mapModal" class="fixed inset-0 backdrop-blur-sm bg-black/30 z-50 hidden items-center justify-center p-4" style="display: none;">
        ...
        if (navigator.geolocation) {
            ...
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    showNotification('Lokasi terdeteksi! Anda dapat menggeser marker atau klik peta untuk mengubah lokasi', 'success');
                },
                function(error) {
                    showNotification('Gagal mendeteksi lokasi. Pastikan Anda telah mengizinkan akses lokasi', 'error');
                }
            );
        } else {
